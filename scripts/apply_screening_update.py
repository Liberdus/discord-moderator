"""Owner-run, hash-pinned 0.4.0 update and approved private screening activation."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.4.0-20260921.pyz')
CONFIGURATOR = Path('/tmp/liberdus-screening-20260921.pyz')
UPDATER_SHA256 = 'cf949a5b343acabbaa067f9dea1012a24e4bafe91c47227a676a6c3896e66735'
CONFIGURATOR_SHA256 = '971a8a9909eccff22bdd81db29bc204b1df45c13f201a219b3845d1f9e1a2664'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-040-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.4.0 code'
            print('2/4 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.4.0':
                raise RuntimeError('Updater did not confirm version 0.4.0.')
            step = 'configuring the approved screening trial'
            print('3/4 Enable test-channel screening: $1/day, $4 total allowance', flush=True)
            result = json.loads(run([sys.executable, str(files[1]), 'screen']))
            if result.get('configured') != 'screen' or result.get('platform_enabled') is not False:
        ...
        print('Existing TypeSafe key reused. A new human message in a test channel can now be screened.')
        return 0
