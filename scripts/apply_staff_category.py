"""Owner-run, hash-pinned 0.5.13 private staff channel category exception; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.13-20260922.pyz')
UPDATER_SHA256 = '0da67b793c7b23a79cb451dc09d1db56d843989977dcb01920a37aa2cf261271'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0513-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.13 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.13':
                raise RuntimeError('Updater did not confirm version 0.5.13.')
        ...
        print('The private bot-mod destination now works inside Committers; other channels remain excluded.')
        return 0
