"""Owner-run, hash-pinned 0.5.5 health and summary update; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.5-20260921.pyz')
UPDATER_SHA256 = '27e7da1ecf1e46775eda9c052ecc0040044f529770b80e737898c0187f4a4e1c'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-055-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.5 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.5':
                raise RuntimeError('Updater did not confirm version 0.5.5.')
        ...
        print('In bot-mod, !mod status shows Version: 0.5.5; !mod summary shows saved operational data.')
        return 0
