"""Owner-run, hash-pinned 0.5.11 shared deletion notices; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.11-20260922.pyz')
UPDATER_SHA256 = 'a6af3760f60a6cb415f499fd45d58a0141ae3f077e69f8aabe5b2d9188abc775'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0511-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.11 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.11':
                raise RuntimeError('Updater did not confirm version 0.5.11.')
        ...
        print('Confirmed deletions now post a shared receipt in bot-mod; no new deletion test is required.')
        return 0
