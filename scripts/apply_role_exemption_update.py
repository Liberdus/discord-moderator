"""Owner-run, hash-pinned 0.4.1 update and approved private screening activation."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.4.1-20260921.pyz')
CONFIGURATOR = Path('/tmp/liberdus-role-exemption-20260921.pyz')
UPDATER_SHA256 = '8bd3ba7bd6a7ad24ca0f6d8eae37586338f4c181d8ebfd0867aabed0ad7c783e'
CONFIGURATOR_SHA256 = 'ec8ed653ca37cd141c1f4c95695ed169c91bc84a34d2cc280904b90e1110dc11'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-041-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.4.1 code'
            print('2/4 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.4.1':
                raise RuntimeError('Updater did not confirm version 0.4.1.')
        ...
        print('JEV now skips observed members of role 1302455329795342377. Repetition checks remain active.')
        return 0
