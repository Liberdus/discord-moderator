"""Owner-run, hash-pinned 0.5.12 live screening diagnostics; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.12-20260922.pyz')
UPDATER_SHA256 = 'f3225c158e5bf639de0246e29c44604efdcb4edbbb85dddeb7a817b2b71baaaa'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0512-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.12 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.12':
                raise RuntimeError('Updater did not confirm version 0.5.12.')
        ...
        print('Live screening failures now retain fixed diagnostics visible in !mod status.')
        return 0
