"""Owner-run, hash-pinned 0.5.15 four-concern auto-delete rule at score >=0.90; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.15-20260922.pyz')
UPDATER_SHA256 = '7899255dcc1bd18d11f0e02654e4cf210715d7c47a62022691f7399f70a8b49b'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0515-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.15 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.15':
                raise RuntimeError('Updater did not confirm version 0.5.15.')
        ...
        print('Auto-delete now covers the four approved concerns at score >=0.90, with existing scope and safety checks.')
        return 0
