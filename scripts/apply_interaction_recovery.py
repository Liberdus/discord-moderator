"""Owner-run, hash-pinned 0.5.14 button acknowledgement and completion recovery; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.14-20260922.pyz')
UPDATER_SHA256 = '44280ab3e88e07074fe38cb72bde459e22f347d1928f98b5b260e15726271608'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0514-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.14 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.14':
                raise RuntimeError('Updater did not confirm version 0.5.14.')
        ...
        print('Button failures now have bounded recovery replies and safe diagnostics; request a fresh confirmation.')
        return 0
