"""Owner-run, hash-pinned 0.5.0 update and approved private moderation action capability."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.0-20260921.pyz')
CONFIGURATOR = Path('/tmp/liberdus-actions-20260921.pyz')
UPDATER_SHA256 = '0af22e8095d9d11dc9f96cc90e2215a1b1fec5390735b4ea6f4af2adf4030501'
CONFIGURATOR_SHA256 = '2d9368ec73d88dd08dfc5917f920a4739c627f43f35ef077724c0983934c0311'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-050-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.0 code'
            print('2/4 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.0':
                raise RuntimeError('Updater did not confirm version 0.5.0.')
            step = 'configuring the approved screening trial'
            print('3/4 Preserve role exemption and enable action capability; runtime flags stay off by default', flush=True)
            ...
            result = json.loads(run([sys.executable, str(files[1]), 'actions']))
            if result.get('configured') != 'actions' or result.get('platform_enabled') is not False:
                raise RuntimeError('Action setup did not confirm the disabled configuration.')
        ...
        print('Run !mod status and !mod help. Deletion, auto-delete and timeout default OFF.')
        print('Grant scoped permissions, then enable each feature explicitly in bot-mod.')
        return 0
