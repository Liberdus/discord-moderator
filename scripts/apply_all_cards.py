"""Owner-run, hash-pinned 0.5.16 consistent Discord cards for all moderation replies; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.16-20260922.pyz')
UPDATER_SHA256 = '09f193ba0e7d8435d1a64908fc87700c78e21d32fa8294fa1ce278ab8ae1c0cc'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-0516-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.16 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.16':
                raise RuntimeError('Updater did not confirm version 0.5.16.')
        ...
        print('Moderation commands, confirmations and private results now use cards; the auto-delete rule is unchanged.')
        return 0
