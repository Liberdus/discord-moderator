"""Owner-run, hash-pinned 0.5.3 interaction reply fix; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.3-20260921.pyz')
UPDATER_SHA256 = '520df1b8c16c5515ad94b7eaac656db1320327ee1215ee8125c77917e0b73b5b'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-053-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.3 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.3':
                raise RuntimeError('Updater did not confirm version 0.5.3.')
        ...
        print('In bot-mod, use !mod incident ID to see the new button layout. Test a staff assessment reply.')
        return 0
