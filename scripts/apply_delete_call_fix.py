"""Owner-run, hash-pinned 0.5.2 deletion fix; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.2-20260921.pyz')
UPDATER_SHA256 = '171bbb5fcb1d5db867e18b9b21f4d9e73acfe5d4b8fe52608ee8418f3f588b02'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-052-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.2 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.2':
                raise RuntimeError('Updater did not confirm version 0.5.2.')
