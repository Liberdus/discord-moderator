"""Owner-run, hash-pinned 0.5.1 deletion fix; preserves policy, runtime flags and budgets."""
import hashlib
import json
import os
...
UPDATER = Path('/tmp/liberdus-update-0.5.1-20260921.pyz')
UPDATER_SHA256 = 'abf3a8c8a5de0784c3b29021f156845a304d0ba7e4bac7fa9481488e37bc2f09'
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='liberdus-apply-051-') as temporary:
            files = []
            ...
            restart(hermes)
            step = 'installing the 0.5.1 code'
            print('2/3 Install the update', flush=True)
            result = json.loads(run([sys.executable, str(files[0])]))
            if result.get('updated') is not True or result.get('version') != '0.5.1':
                raise RuntimeError('Updater did not confirm version 0.5.1.')
        ...
        print('Test with a new message in bot-test-1; older incidents are not automatically retried.')
        return 0
