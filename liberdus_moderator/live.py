...
class LiveSession:
    ...
    def command(self, event, message_id, connected):
        ...
        if event.command == "review" and event.reply_to_message_id is not None:
            target = self.review_reply_target(event)
            if target is None or len(event.arguments) != 1:
                return "Review not saved. Use a recorded moderation report or incident view; choose a review button or reply with !mod review promotion, not-promotion, or unsure."
        ...
        if not response["ok"]:
            ...
            return "Moderation command: " + response["error"] + ". Use !mod status, incident ID, review ID REV LABEL, pause, resume, or selftest."
        ...
        if event.command == "review":
            ...
            return (f"**Moderator review saved**\nLabel: {label}\n"
                    ...
                    "Human label only. JEV result retained; enforcement disabled.\n"
