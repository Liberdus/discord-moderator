...
class LiveSession:
    ...
    def command(self, event, message_id, connected):
        ...
        if not response["ok"]:
            ...
            return panel("Moderation command", [response["error"].rstrip("."), "Use !mod help for commands."])
        ...
        if event.command == "connection":
            from .connection_health import summary
            return summary(self.engine, connected)
        if event.command == "help":
            from .display import panel
            return panel("Moderation help", [
                "PRIVATE STAFF COMMANDS", "--------------------------------",
                "!mod status", "Show connection and JEV usage.", "",
                "!mod summary", "Show saved counts and cost scope.", "!mod connection", "Show reconnect history.", "",
                ...
                "!mod delete ID REV [MESSAGE_ID]", "Fetch current text; then confirm.", "!mod timeout ID REV", "!mod actions ID", "Confirm with the shown button.",
