...
The private lookup and new reply format have been confirmed. The [JEV batch runner](jev-batch.md) now automates the next comparison using ten synthetic incident examples and the real provider, without posting across test channels. The owner completed it: ten valid responses, nine matches and one mixed-purpose disagreement. The [baseline review](jev-batch.md#baseline-results--september-20-2026) records the evidence. The completed [precedence-v2 follow-up](jev-batch.md#precedence-v2-results--september-20-2026) returned five valid responses and four matches; an ambiguous fragment remains a review case. The candidate has not changed live incident lookups. No full baseline rerun is needed. Ordinary messages that never trigger a rule are not evaluated by this integration. The three completed examples establish connectivity and initial behavior, not general accuracy or confidence calibration.
