...
**0.3.4 prepared, September 21, 2026:** review buttons, private human labels, saved-text previews and explicit message links are implemented and tested offline. The owner-run update and live display/click check are pending; follow [the current runbook](moderator-review.md). The last confirmed live plugin is 0.3.3.
...
- Reports and command responses disable mentions, embeds, and notifications. Automatic reports contain IDs and jump links. Version 0.3.4 incident commands additionally show an escaped saved-text preview in the private channel. Reports retain their displayed evidence revision. In 0.3.5 an authorized staff assessment attempts to refresh its clicked bot message and latest private report; source edits/deletions alone do not rewrite every old Discord post. Use the incident command to inspect current status.
...
The three initial [JEV shadow comparisons](jev.md#initial-trial-complete) are now complete; neither these examples nor the passed code-only cases need repeating. The owner has confirmed the private saved-result lookup and its 0.3.3 formatting. The owner also completed the [JEV batch baseline](jev-batch.md#baseline-results--september-20-2026): ten valid responses, nine matches and one mixed-purpose review case. The owner also completed the [precedence-v2 candidate comparison](jev-batch.md#precedence-v2-results--september-20-2026): five valid responses, four matches and one ambiguous-fragment review case. It has not changed the live rubric. These classification comparisons do not replace remaining live Discord acceptance checks. Continue normal-message and false-positive observation in the existing test channels. Record additional evidence when available; a broader channel pilot and enforcement remain later decisions.
...
## Human review feature prepared — September 21, 2026

The operator selected Promotion / Not promotion / Unsure buttons. The 0.3.4 source includes private click authorization, persistent displayed-revision bindings, bounded human audit history, source previews/links, and an owner-run updater. Offline validation passed; no live profile access, key access, Discord posts, provider calls or gateway restart was performed during this implementation. Installation and a fresh lookup/click of a retained incident remain pending. A later real restart should confirm deployed button recovery when convenient; the stored-binding and new-client handler paths are covered offline. Existing live checks do not need repetition.

## Staff assessment workflow prepared — September 21, 2026

The operator approved high-level assessment labels, visible reviewer/status updates, a pending list and the proposed two-box formatting. Version 0.3.5 implements these with preserved old content labels, no automatic enforcement or training, and no new provider call for staff operations. Follow the new runbook to install and inspect an existing incident. Completed reviews are retained as audit data for later deliberate rule/rubric evaluation; this release does not tune JEV automatically. Offline validation passed, but no live profile, provider or Discord call was made by the developer.
