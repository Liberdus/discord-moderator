...
**Selected follow-up:** a separate [precedence-v2 candidate comparison](jev-batch.md#next-focused-precedence-v2-comparison) now tests five examples with explicit mixed-content precedence. It uses the same model, transport and shared caps while the live 0.3.3 classifier keeps its original rubric. Real candidate results are pending owner execution; compare them before adopting the candidate for live incidents.
