...
**Current status — September 20, 2026: initial three-case trial complete.** All three operator-supplied local records have `outcome: ok` and their intended labels: announcement, promotion and quoted warning. Accounting records three attempts; the latest result took 375 ms. This verifies these examples and the live integration, not general accuracy or calibrated confidence. The supplied mode remains `shadow`; documentation updates did not change the live profile. Completed setup/tests need no repetition. Keep the prescribed trial limits and disabled enforcement unchanged. The owner has also confirmed the 0.3.2 saved-result lookup in the private moderator channel. The owner subsequently supplied the [0.3.3 formatted reply](incident-review.md), confirming the narrow ASCII panel and retained result. This lookup/display step is complete.

Implemented in 0.3.0; **disabled in the installation bundle**. The chosen scope is existing code-rule incidents only. This does not scan every message, create new incidents, suppress rule reports, annotate Discord reports, invoke Hermes's model/tools, or perform moderation actions. This describes `shadow` mode. Version 0.4.0 separately supports `report_only` single-message screening; see the current runbook above.
...
- `!mod status` shows JEV mode/state and cumulative AI **attempts**, including reserved/uncertain attempts. Code-rule decisions still correctly report zero synchronous AI calls: evaluation is separate. Results are saved locally; 0.3.2 also displays them on demand in authorized private incident replies. Automatic rule reports remain unchanged.
...
All three patterns should still produce ordinary code-rule reports regardless of JEV's choice. JEV results are stored locally; 0.3.2 exposes those saved results through authorized private incident lookups. There is no automatic JEV post or change to the rule report. Use a fresh phrase for any later retest because evaluations are limited to one attempt per incident. Earlier incidents are not automatically evaluated when the flag is enabled. These examples test connectivity and initial label behavior, not classification accuracy across the server.
...
**Current runtime:** the operator's result still reports `mode: shadow`; this checkpoint did not turn it off or change the live profile. While shadow remains enabled, future eligible incidents in the approved test scope may produce provider calls under the existing limits. The [stop procedure](#stop-the-trial) restores code-only monitoring when desired. At this trial checkpoint classifications were available through local records. The subsequently deployed 0.3.2 lookup now provides authorized private inspection, confirmed by the owner; fixed automatic reports still come from code rules and enforcement remains disabled.

**Private lookup and 0.3.3 formatted reply confirmed:** the operator supplied a working `!mod incident b48ee236ff774e0f8efec35caa8ba447` reply: incident revision 2, `needs_revalidation`, saved `quoted_warning`, confidence 1.00, evaluated revision 1, age 40m, historical after closure/window reset. The reply stated no new AI call. Version 0.3.3 presents the same metadata in a 32-column ASCII panel with copyable IDs below, for both `!mod incident` and `!mod explain`. The lookup keeps authorization and makes no provider call or change to report selection, suppression or enforcement. The owner subsequently supplied that incident in the new format, with age 1h, both IDs, `Needs recheck`, `HISTORICAL` and the no-new-call footer. The [runbook](incident-review.md) records this completed display check; no update or paid example needs repeating.
...
**Selected follow-up completed:** the owner supplied [precedence-v2 results](jev-batch.md#precedence-v2-results--september-20-2026): five valid responses and four matches. The ambiguous forwarded fragment returned promotion at displayed score 0.65 instead of unclear. Mean recorded latency was 296.8 ms, the known token estimate was $0.000144, the reservation was $0.013765, and shared accounting now reports 18 calls. Preserve this review case and the original 9/10 baseline. The candidate remains experimental while the live 0.3.3 classifier keeps its original rubric in shadow mode; no repeat run or runtime change is needed now.
