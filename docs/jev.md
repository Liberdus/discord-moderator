...
**Private lookup confirmed; 0.3.3 layout update ready:** the operator supplied a working `!mod incident b48ee236ff774e0f8efec35caa8ba447` reply: incident revision 2, `needs_revalidation`, saved `quoted_warning`, confidence 1.00, evaluated revision 1, age 40m, historical after closure/window reset. The reply stated no new AI call. Version 0.3.3 presents the same metadata in a 32-column ASCII panel with copyable IDs below, for both `!mod incident` and `!mod explain`. The lookup keeps authorization and makes no provider call or change to report selection, suppression or enforcement. Follow the [update runbook](incident-review.md) and inspect this existing incident to check the layout on the phone; no paid example needs repeating.
