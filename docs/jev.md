...
**Current runtime:** the operator's result still reports `mode: shadow`; this checkpoint did not turn it off or change the live profile. While shadow remains enabled, future eligible incidents in the approved test scope may produce provider calls under the existing limits. The [stop procedure](#stop-the-trial) restores code-only monitoring when desired. At this trial checkpoint classifications were available through local records. The subsequently implemented 0.3.2 lookup adds authorized private inspection after deployment; fixed automatic reports still come from code rules and enforcement remains disabled.

**Selected next improvement — implemented in 0.3.2, deployment pending:** the operator requested the saved-result lookup. `!mod incident <id>` and `!mod explain <id>` now add stored typed JEV data, evaluated revision, age and current/historical evidence status while preserving command authorization. The lookup makes no provider call and does not change report selection, suppression or enforcement. Follow the [update and next-step runbook](incident-review.md) to deploy it and inspect one of the existing trial incidents.
