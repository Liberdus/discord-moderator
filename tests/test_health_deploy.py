...
from scripts import apply_health_update as helper


class DeploymentTests(unittest.TestCase):
    def invoke(self, *, failure=None, wrong_hash=False, user='hermes', reported_version='0.5.5'):
        calls=[]
        with tempfile.TemporaryDirectory() as temp:
            ...
            def run(argv):
                ...
                if argv[-1].endswith('update.pyz'): return '{"updated":true,"version":"'+reported_version+'"}'
                return ''
    ...
    def test_previous_release_result_never_reenables_moderation(self):
        result,calls=self.invoke(reported_version='0.5.4')
        self.assertEqual(result,2)
        self.assertEqual(len(calls),3)
        self.assertFalse(any(call[-1]=='true' for call in calls))
