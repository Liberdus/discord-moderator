...
from scripts import apply_actions_update as helper
...
class DeploymentTests(unittest.TestCase):
    def invoke(self, *, failure=None, wrong_hash=False, user='hermes'):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            def run(argv):
                ...
                if argv[-1]=='actions': return '{"configured":"actions","platform_enabled":false}'
                if argv[-1]=='exempt-role': return '{"configured":"exempt-role","platform_enabled":false}'
                if argv[-1].endswith('update.pyz'): return '{"updated":true,"version":"0.5.0"}'
                return ''
    ...
    def test_success_updates_then_configures_before_enabling(self):
        ...
        self.assertEqual(calls[4][-1],'actions')
        self.assertEqual(calls[5][-1],'true')
        self.assertEqual(calls[6][-1],'restart')
    ...
    def test_any_failure_stops_later_steps(self):
        for stage in range(1,8):
            ...
            if stage<=5: self.assertFalse(any(call[-1]=='true' for call in calls))
