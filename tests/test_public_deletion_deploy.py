...
spec = importlib.util.spec_from_file_location('deletion_apply', ROOT/'scripts/apply_public_deletion.py')
helper = importlib.util.module_from_spec(spec)
spec.loader.exec_module(helper)


class PublicDeletionDeployTests(unittest.TestCase):
    def setUp(self):
    ...
    def _run(self,args):
        ...
        if args[-1]=='verify': return json.dumps(dict(verified=True,public_deletion_allowed=True))
        if args[-1]=='apply':
            if self.failure=='apply': raise RuntimeError('scope changed during restart')
            return json.dumps(dict(configured=True,actions_enabled=True,public_deletion_allowed=True,action_flags_reset=True,platform_enabled=False))
        if args[-1]=='restart':
        ...
        if len(args)==2 and args[1].endswith('update.pyz'):
            ...
            return json.dumps(dict(updated=True,version='0.5.8'))
        return ''
