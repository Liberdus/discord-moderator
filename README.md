...
Version 0.3.1 adds an operator-only `!mod selftest` command: nine fixed synthetic checks, isolated from live moderation state. The existing pilot needs the [owner-run code update](docs/selftest.md#update-the-existing-030-pilot) before that command is available. Self-tests do not mark pending live Discord checks as passed; JEV remains off.
