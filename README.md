...
1. Review the completed [three-case JEV shadow trial](docs/jev.md#initial-trial-complete). A proposed next improvement is to expose saved results through the authorized private `!mod incident` lookup, avoiding repeated SSH checks; this is not implemented or an instruction to change report behavior.
