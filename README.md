...
**Initial JEV shadow trial complete: three examples matched.** Announcement, promotion and quoted warning all returned `ok` with their intended labels; recorded latencies were 275, 318 and 375 ms. Accounting records three attempts. This verifies the initial examples and live integration, not general accuracy or calibrated confidence. See the [trial checkpoint](docs/jev.md#initial-trial-complete). Version **0.3.2** now adds saved JEV results to authorized private `!mod incident` / `!mod explain` replies, including historical-evidence status and no new provider call. The [owner-run update and verification](docs/incident-review.md) are ready; deployment is pending. Automatic reports remain rule-based, enforcement stays disabled, and installation defaults remain off.
...
1. Apply the [0.3.2 code update](docs/incident-review.md) and inspect an existing trial incident in `bot-mod`; this verifies the new saved-result display without another provider call. Then review varied, labeled rule-triggering examples in the existing test scope.
