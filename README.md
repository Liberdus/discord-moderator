...
**JEV batch runner ready:** one owner-run command evaluates ten synthetic examples against JEV and prints expected/actual labels, latency and cost accounting. It shares the existing pilot budget, saves attempts for reuse, and needs no plugin update or gateway restart. [Run the batch](docs/jev-batch.md); live batch results remain pending.
...
1. Run the [JEV batch evaluation](docs/jev-batch.md) once from the existing hermes terminal, then review label disagreements. The formatted incident lookup is already confirmed. The batch uses synthetic text, the real JEV API and the existing spending caps; no Discord posts or restart are needed.
