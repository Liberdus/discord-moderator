...
**JEV shadow trial authorized; activation and first provider result pending.** The operator now has a TypeSafe key and selected a small trial in the existing private test channels. The last supplied runtime status still shows JEV off and zero AI attempts. Follow the [existing-installation JEV setup](docs/jev.md#owner-run-setup-on-db2); no plugin reinstall is needed. Shadow evaluation classifies existing rule incidents into local records, leaving Discord reports unchanged. Enforcement remains disabled. Installation defaults remain off.
...
1. Run the authorized [small JEV shadow trial](docs/jev.md#first-live-shadow-trial) using fresh, deliberately written human messages in the existing test channels. Confirm one successful provider result before comparing more examples.
...
[BUILD_PLAN.md](docs/BUILD_PLAN.md) retains the historical planning baseline with dated additions. Section 10.6 records the original JEV proposal; section 10.7 records the selected incident-only shadow implementation. Section 10.8 records the earlier deferral; section 10.13 records the subsequent live checks and the authorized JEV shadow trial, still awaiting activation evidence. [JEV instructions](docs/jev.md) cover API keys, budgets, and activation. Its original status statements and local workstation links are preserved; this README and the operations guide describe current implementation status. It also records feature ideas from Sentinel AI, Defender/Warden, Zeppelin, Modcord, and Omnicord. This initial core is independently implemented; no third-party bot source has been imported.
