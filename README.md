...
**Focused JEV comparison ready:** the separate `precedence-v2` candidate prioritizes an endorsed sales pitch within mixed content while keeping quoted warnings distinct. [Run five targeted cases](docs/jev-batch.md#next-focused-precedence-v2-comparison) using the existing key and caps. The live classifier still uses its original rubric; no plugin installation or restart is needed.
...
1. Run the [five-case precedence-v2 comparison](docs/jev-batch.md#next-focused-precedence-v2-comparison) and review the output before promoting the revised rubric to live incidents. The original 9/10 baseline is preserved. The candidate shares the existing shadow budget; no installation or restart is needed.
