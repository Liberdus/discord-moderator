...
**Focused JEV comparison completed:** `precedence-v2` returned five valid responses and four expected-label matches. The ambiguous forwarded fragment returned promotion at displayed score 0.65 instead of unclear and remains a review case. Recorded latency averaged 296.8 ms; shared accounting now reports 18 calls. See the [candidate results](docs/jev-batch.md#precedence-v2-results--september-20-2026). The candidate remains experimental; the live classifier still uses its original rubric. The completed comparison needs no repeat run. The separately prepared 0.3.4 plugin update below has its own installation step.

**0.3.4 prepared — September 21, 2026:** private reports and incident views now offer **Promotion / Not promotion / Unsure** buttons. Incident views include an escaped saved-text preview and clickable source links; human reviews are stored separately from JEV and tied to the displayed evidence revision. The last confirmed live version is still 0.3.3. Follow the [one-command owner update and existing-incident check](docs/moderator-review.md); deployment and live button verification are pending.
...
- Private moderator review buttons, bounded audit history, revision-safe saved message previews and explicit source links. Reviews do not change rule/JEV decisions or enable actions.
...
1. Install the prepared 0.3.4 update and verify the [review buttons and saved evidence](docs/moderator-review.md) using an existing incident. Collect human judgments on useful examples and JEV disagreements. Keep the original 9/10 baseline, candidate 4/5 comparison, live context-v1 rubric and existing shadow limits; another paid comparison is not needed for this release.
...
The successful repeat report proves the basic path, not every integration case or all live phases. Protected-role enforcement, general Hermes conversation, AI-generated rule candidates and action reconciliation remain future work.
