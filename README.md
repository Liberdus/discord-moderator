...
**Two live JEV shadow comparisons passed.** The announcement and promotion examples both returned `ok` with their expected labels, at recorded latencies of 275 ms and 318 ms respectively. Local accounting records two attempts. These examples confirm the observed integration behavior, not general classification accuracy. Next is the quoted-warning comparison in the [JEV trial](docs/jev.md#first-live-shadow-trial). Shadow results remain local, Discord reports remain rule-based, enforcement stays disabled, and installation defaults remain off. No reinstall or setup repeat is needed.
...
1. Continue the [small JEV shadow trial](docs/jev.md#first-live-shadow-trial): announcement and promotion returned `ok` with their expected labels. Compare the quoted-warning example next, using human messages in the existing test channels.
...
The inactive [CI template](docs/ci-workflow.yml) runs the tests and fixture commands on Python 3.11, 3.12, and 3.13, with read-only repository permissions and no secrets. The 115 core/setup tests pass on Python 3.11.16 and 3.12.3; 29 additional integration tests use the real c1488 Hermes source and discord.py 2.7.1 with network edges mocked. Those integration tests are separate from the CI template and do not constitute a live gateway test; GitHub CI is not enabled yet because the current GitHub CLI login lacks the separate `workflow` scope. Once authorized, move the template to `.github/workflows/ci.yml`. See [operations](docs/operations.md), [recovery](docs/recovery.md), and [integration](docs/integration.md).

[BUILD_PLAN.md](docs/BUILD_PLAN.md) retains the historical planning baseline with dated additions. Section 10.6 records the original JEV proposal; section 10.7 records the selected incident-only shadow implementation. Section 10.8 records the earlier deferral; section 10.13 records the subsequent live checks and trial authorization; sections 10.14 and 10.15 record the successful announcement and promotion results. [JEV instructions](docs/jev.md) cover API keys, budgets, and activation. Its original status statements and local workstation links are preserved; this README and the operations guide describe current implementation status. It also records feature ideas from Sentinel AI, Defender/Warden, Zeppelin, Modcord, and Omnicord. This initial core is independently implemented; no third-party bot source has been imported.
